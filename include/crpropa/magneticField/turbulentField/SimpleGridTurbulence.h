...
class SimpleTurbulenceSpectrum : public TurbulenceSpectrum {
	protected:
		inline static const int constScaleBendover = 1000; 	// define the bandover scale as 1000 * lMax to ensure k * lBendover >> 1. The bendover scale is necessary for the implementation of PlaneWaveTurbulence. 
	...
	public:
		...
		* The bend-over scale is set to 1000 times lMax to ensure to be in the inertial range. This should not be changed.
		 ...
		 */
		SimpleTurbulenceSpectrum(double Brms, double lMin, double lMax, double sIndex = 5. / 3) : TurbulenceSpectrum(Brms, lMin, lMax, constScaleBendover * lMax, sIndex, 0) {
		}
		...
		*/
		...
		double getCorrelationLength() const {
			return turbulentCorrelationLength(getLmin(), getLmax(), getSindex());
		}
...
inline double turbulentCorrelationLength(double lMin, double lMax, double alpha = -11 / 3.) {
	KISS_LOG_WARNING << "turbulentCorrelationLength is deprecated and will be removed in the future. Replace it with a more appropriate turbulent field model and call getCorrelationLength().";
	return SimpleTurbulenceSpectrum::turbulentCorrelationLength(lMin, lMax, -alpha - 2);
}
...
inline void initTurbulence(ref_ptr<Grid3f> grid, double Brms, double lMin, double lMax, double alpha = -11 / 3., int seed = 0) {
	KISS_LOG_WARNING << "initTurbulence is deprecated and will be removed in the future. Replace it with a more appropriate turbulent field model instance.";
