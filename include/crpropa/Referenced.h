 ...
 *
 * A form of memory management is needed to prevent memory leaks when using CRPropa in Python via SWIG.
...
class Referenced {
	public:
		...
		inline Referenced() : _referenceCount(0) {
		}

		inline Referenced(const Referenced&) : _referenceCount(0) {
		}
		...
		inline size_t removeReference() const {
				...
				if (_referenceCount == 0)
					std::cerr << "WARNING: Remove reference from Object with NO references: " << typeid(*this).name() << std::endl;
			#endif
				int newRef;
			#if defined(OPENMP_3_1)
				#pragma omp atomic capture
	...
	protected:
		...
		virtual inline ~Referenced() {
			#ifdef DEBUG
				if (_referenceCount)
					std::cerr << "WARNING: Deleting Object with references: " << typeid(*this).name() << std::endl;
			#endif
		}
...
class ref_ptr {
	public:
		...
		T* release() {
			T* tmp = std::exchange(_ptr, nullptr);
			if (_ptr)
		...
		void swap(ref_ptr& rp) {
			T* tmp = std::move(_ptr);
			_ptr = std::move(rp._ptr);
			rp._ptr = std::move(tmp);
		}
