...
constexpr int reflectiveBoundary(int index, int n) {
...
constexpr int periodicBoundary(int index, int n) {
	return ((index % (n)) + (n)) % (n);
}

/** Lower and upper neighbour in a reflectively repeated unit grid */
constexpr void reflectiveClamp(double x, int n, int& lo, int& hi, double& res) {
...
constexpr double round(double r) {
...
public:
	...
	virtual ~GridProperties() = default;
	...
	public:
		...
		*/
		...
		void setInterpolationType(interpolationType ipolType) {
			if (ipolType == TRILINEAR or ipolType == TRICUBIC or ipolType == NEAREST_NEIGHBOUR) {
