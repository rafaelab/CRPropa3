...
class SynchrotronRadiation: public Module {
	protected:
		...
		int maximumSamples; ///< maximum number of samples of synchrotron photons (break condition; 0 or <0 means no sampling)
	...
	public:
		 ...
		 */
		SynchrotronRadiation(ref_ptr<MagneticField> field, bool havePhotons = false, ref_ptr<SamplerEvents> sampling = ref_ptr<SamplerEvents>(new SamplerEventsNull()), ref_ptr<SamplerDistribution> samplingDist = nullptr, int maximumSamples = 0, double limit = 0.1);
		/** Constructor
		 ...
		 * @param limit			step size limit as fraction of mean free path
		 */
		SynchrotronRadiation(double Brms = 0, bool havePhotons = false, ref_ptr<SamplerEvents> sampling = ref_ptr<SamplerEvents>(new SamplerEventsNull()), ref_ptr<SamplerDistribution> samplingDist = nullptr, int nSamples = 0, double limit = 0.1);
		...
		ref_ptr<SamplerEvents> getSamplerEvents() const;
		ref_ptr<SamplerDistribution> getSamplerDistribution() const;
