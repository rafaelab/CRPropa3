 ...
 * Vetoes for more than one type of particle can be added by calling this feature multiple times.
 */
 ...
 * This observer is very useful if the time evolution of the particle density is needed. 
 * It detects all candidates in lin-spaced, log-spaced, or user-defined time intervals and limits the nextStep of candidates to prevent overshooting of detection intervals.
 */
class ObserverTimeEvolution: public ObserverFeature {
	...
	public:
		...
		/** Function
		 * Generates the detList if it is empty when for example the ObserverTimeEvolution(const std::vector<double>& detList) constructor was not used.
