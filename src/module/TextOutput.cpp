...
TextOutput::TextOutput(std::ostream& out, OutputType outputtype) : 
	Output(outputtype), out(&out), storeRandomSeeds(false) {
}
...
void TextOutput::process(Candidate* c) const {
	if (fields.none() and properties.empty())
		return;
	...
	if (fields.test(TrajectoryLengthColumn))
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->getTrajectoryLength() / lengthScale);
	if (fields.test(RedshiftColumn))
	...
	if (fields.test(CurrentEnergyColumn))
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->current.getEnergy() / energyScale);
	if (fields.test(CurrentPositionColumn)) {
		if (oneDimensional) {
			p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->current.getPosition().x / lengthScale);
		} else {
			const Vector3d pos = c->current.getPosition() / lengthScale;
			p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t%8.5E\t%8.5E\t", pos.x, pos.y, pos.z);
		}
	}
	...
	if (fields.test(SourceEnergyColumn))
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->source.getEnergy() / energyScale);
	if (fields.test(SourcePositionColumn)) {
		if (oneDimensional) {
			p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->source.getPosition().x / lengthScale);
		} else {
	...
	if (fields.test(CreatedEnergyColumn))
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->created.getEnergy() / energyScale);
	if (fields.test(CreatedPositionColumn)) {
		if (oneDimensional) {
			p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->created.getPosition().x / lengthScale);
		} else {
	...
	if (fields.test(WeightColumn)) {
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%8.5E\t", c->getWeight());
	}
	if (fields.test(CandidateTagColumn)) {
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%s\t", c->getTagOrigin().c_str());
	}
	...
	for (std::vector<Output::Property>::const_iterator iter = properties.begin(); iter != properties.end(); ++iter) {
		...
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "%s", v.toString("\t").c_str());
		p += std::snprintf(buffer + p, sizeof(buffer) - p, "\t");
	}
	buffer[p - 1] = '\n';
