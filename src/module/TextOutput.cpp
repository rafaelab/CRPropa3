...
void TextOutput::process(Candidate* c) const {
	...
	if (fields.test(TrajectoryLengthColumn))
		p += std::sprintf(buffer + p, "%8.5E\t", c->getTrajectoryLength() / lengthScale);
	if (fields.test(RedshiftColumn))
		p += std::sprintf(buffer + p, "%1.5E\t", c->getRedshift());
	if (fields.test(SerialNumberColumn))
		p += std::sprintf(buffer + p, "%10lu\t", c->getSerialNumber());
	if (fields.test(CurrentIdColumn))
		p += std::sprintf(buffer + p, "%10i\t", c->current.getId());
	if (fields.test(CurrentEnergyColumn))
		p += std::sprintf(buffer + p, "%8.5E\t", c->current.getEnergy() / energyScale);
	if (fields.test(CurrentPositionColumn)) {
		if (oneDimensional) {
			p += std::sprintf(buffer + p, "%8.5E\t", c->current.getPosition().x / lengthScale);
		} else {
	...
	if (fields.test(SourceEnergyColumn))
		p += std::sprintf(buffer + p, "%8.5E\t", c->source.getEnergy() / energyScale);
	if (fields.test(SourcePositionColumn)) {
		if (oneDimensional) {
			p += std::sprintf(buffer + p, "%8.5E\t", c->source.getPosition().x / lengthScale);
		} else {
	...
	if (fields.test(SerialNumberColumn))
		p += std::sprintf(buffer + p, "%10lu\t", c->getCreatedSerialNumber());
	if (fields.test(CreatedIdColumn))
		p += std::sprintf(buffer + p, "%10i\t", c->created.getId());
	if (fields.test(CreatedEnergyColumn))
		p += std::sprintf(buffer + p, "%8.5E\t", c->created.getEnergy() / energyScale);
	if (fields.test(CreatedPositionColumn)) {
		if (oneDimensional) {
			p += std::sprintf(buffer + p, "%8.5E\t", c->created.getPosition().x / lengthScale);
		} else {
